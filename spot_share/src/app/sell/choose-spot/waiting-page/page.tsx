...
export default function WaitingPage() {
  ...
  const [recentActivity, setRecentActivity] = useState<Activity[]>([
    {
      ...
      type: "view",
      time: "2 mins ago",
      message: "Someone viewed your spot",
    },
    {
      id: 2,
      type: "view",
        ...
        if (prev <= 1) {
          ...
          setTimeout(() => {
            window.location.href =
              "http://localhost:3000/sell/choose-spot/waiting-page/eta";
  ...
  const initMap = () => {
    ...
    try {
      ...
      const redPulseOverlay = new RedPulseOverlay(
        new window.google.maps.LatLng(parkingLot.lat, parkingLot.lng)
      );
      ...
      const bluePulseOverlay = new BluePulseOverlay(
        new window.google.maps.LatLng(sellerLocation.lat, sellerLocation.lng)
      );
  ...
  const statusDotClasses = matchFound
    ? "w-3 h-3 rounded-full bg-green-300"
    : "w-3 h-3 rounded-full bg-green-400 animate-pulse";
  const statusTextClasses = matchFound
    ? "text-lg text-green-100"
    : "text-lg text-blue-100";
  const timerLabelClasses = matchFound
    ? "text-sm mb-1 text-green-100"
    : "text-sm mb-1 text-blue-100";
